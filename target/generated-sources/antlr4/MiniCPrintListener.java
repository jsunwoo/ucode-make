...
public class MiniCPrintListener extends MiniCBaseListener {
	...
	public void exitFun_decl(MiniCParser.Fun_declContext ctx) {
		...
		if (ctx.getChildCount() == 6) {
			...
			stmt += "\n";
			newTexts.put(ctx, stmt); // 마무리 다포함
			// System.out.println(stmt);//
			// procLine = "proc  " + numOfFunc + " " + numOfLocal + " 2 ";
			// newTexts.put(ctx, procLine);
		}
		// tab--;
	}
	...
	public void exitCompound_stmt(MiniCParser.Compound_stmtContext ctx) {
		...
		if (ctx.getChildCount() >= 2) {
			...
			for (int i = 1; i < ctx.getChildCount() - 1; i++) {
				// stmt += printTab();
				if (ctx.local_decl().contains(ctx.getChild(i))) // local_decl인
																// 경우
	...
	public void exitLocal_decl(MiniCParser.Local_declContext ctx) {
		...
		symLine = "sym   " + numOfFunc + " " + numOfLocal + " 1\n";
		newTexts.put(ctx, symLine);
		// System.out.println(symLine);
		...
		if (ctx.getChildCount() > 0) {
			...
			// '!' expr | '-' expr | '+' expr일 경우
				...
				// IDENT '=' expr
					...
					// System.out.println("ldc\t" + newTexts.get(ctx.expr(0)));
					// System.out.println("str\t" + "2\t" +
					// ctx.getChild(0).getText());
					expr = "ldc   " + newTexts.get(ctx.expr(0)) + "\nstr   "
							+ "2 " + ctx.getChild(0).getText();
